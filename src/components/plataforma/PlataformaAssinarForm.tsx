...
const PLANOS = {
  ...
  growth: { label: "Growth", conversas: "500 conversas/mês", preco: "R$699/mês" },
  scale: { label: "Scale", conversas: "1.000 conversas/mês", preco: "R$999/mês" },
} as const;

const TAXA_AGENTE_PRONTO = 1000;
...
export function PlataformaAssinarForm() {
  const searchParams = useSearchParams();
  const planoParam = searchParams.get("plano");
  const plano: PlanoKey | null =
    planoParam && planoParam in PLANOS ? (planoParam as PlanoKey) : null;
  const agentePronto = searchParams.get("agentePronto") === "1";
  ...
  const [error, setError] = useState<string | null>(
    erro ? "Não foi possível concluir o checkout da assinatura. Tenta de novo." : null
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/plataforma-subscription", {
        method: "POST",
          ...
          {!plano ? (
            <div className="card-surface rounded-3xl p-8 text-center">
              ...
              <p className="text-muted mb-6">
                Não identificamos qual plano você quer assinar. Volta pra
                página de planos e clica em um dos botões de assinatura.
              </p>
              <Link href="/plataforma#planos" className="text-accent font-semibold hover:underline">
                Ver planos e preços
              </Link>
            </div>
          ) : (
            <>
              <div className="mb-8">
                <p className="text-xs font-semibold uppercase tracking-widest text-accent mb-2">
                  ...
                  {agentePronto && " + Agente Pronto"}
                </p>
                <h1 className="text-2xl sm:text-3xl font-black tracking-tight mb-1">
                  {agentePronto
                    ? `R$${TAXA_AGENTE_PRONTO} (taxa única) + ${PLANOS[plano].preco}`
                    : PLANOS[plano].preco}
                </h1>
                <p className="text-muted text-sm">
                  {PLANOS[plano].conversas}
                  {agentePronto &&
                    " — a taxa é cobrada primeiro; depois de paga, você segue direto pro checkout da assinatura mensal."}
                </p>
              </div>
